...
public class BookRegistrationTool {
    ...
    public record Request(Long patientId, Long scheduleId) {}

    @Bean
    @Description("患者确认要挂号时调用此工具。必须提供 patientId 和 scheduleId。调用成功即代表挂号成功。")
    public Function<Request, String> bookRegistration() {
        return request -> {
            if (request.patientId() == null || request.scheduleId() == null) {
                return "挂号失败：patientId 和 scheduleId 不能为空";
            }
            try {
                RegistrationCreateReq req = new RegistrationCreateReq();
                req.setPatientId(request.patientId());
