...
import com.neusoft.neu23.neuhospital.integration.patient.RemotePatientUpdateRequest;
...
public class UpdatePatientInfoTool {
    ...
    public UpdatePatientInfoTool(PatientGatewayClient patientGatewayClient) {
    ...
    public Function<Request, String> updatePatientMemory() {
        return request -> {
            try {
                ...
                // 先查询已有数据
                RemotePatientProfile patient = patientGatewayClient.getPatientById(patientId);
                if (patient == null) {
                    return "更新失败：未找到当前会话绑定的患者记录。";
                }

                RemotePatientUpdateRequest updateReq = new RemotePatientUpdateRequest();
                ...
                patientGatewayClient.updatePatient(patientId, updateReq);
